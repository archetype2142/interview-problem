class Calculations
  ...
  def process_discount_rule
    discount_rule = promo_rules.select { |rule| rule.type.to_sym == :vol_discount }
  ...
  def process_multiple_buys
    multiple_buy_rules = promo_rules.select { |rule| rule.type.to_sym == :multiple_buy }
    return if multiple_buy_rules.empty?
    multiple_buy_rules.each do |rule|
      update_unit_prices(
        rule.params[:code], 
        rule.params[:quantity],
